...
void FstFile::value_change_callback(void * user_callback_data_pointer, uint64_t time, handle_t facidx,
    ...
    while((hier = fstReaderIterateHier(reader))) {
        ...
        case FST_HT_SCOPE: {
            if(in_node_scope) {
                ...
                current_node_data = &current_node_data->subscopes.try_emplace({scope.name}, NodeData{}).first->second;
            }
            if(next_is_node) {
                next_is_node  = false;
                in_node_scope = true;
        ...
        case FST_HT_VAR: {
            if(in_node_scope) {
                ...
                if(var.length == 1) { formatter.reset(new BinaryFormatter{}); }
                current_node_data->variables.insert(
                    {{var.name}, {var.name, var.length, var.handle, node, std::move(formatter)}});
