...
std::span<char> FixedFormatter::format(char * signal) {
    ...
    while(cache_cleaned.back() == ' ') { cache_cleaned.pop_back(); }
...
bigint FixedFormatter::visit_stmt(const impl::Const & c, const bigint &) { return c.value; }
bigint FixedFormatter::visit_stmt(const impl::Signal &, const bigint & s) { return s; }
    ...
    for(const auto & c : sv.cases) {
        if(c.cases.size() == 0) {
            ...
            for(const auto & [expected, mask] : c.cases) {
                if((test & mask) == expected) { return visit_stmt(c.value, s); }
            }
        }
    }
    assert(false);
...
bigint FixedFormatter::bin_to_bigint(const char * signal) {
    ...
    size_t pos = 0;
    while(signal[pos] != '\0') {
        if(signal[pos] == '1') { boost::multiprecision::bit_set(i, len - pos - 1); }
        pos++;
    }
    return i;
}

void from_json(const json & j, FixedFormatter & n) { j.get_to(n.chunks); }
FixedFormatter parse_formatter(std::string fmt) { return json::parse(fmt).template get<FixedFormatter>(); }
