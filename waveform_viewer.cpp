	...
	// waveform labels
	{
		...
		while (clipper.Step()) {
			for (int i = clipper.DisplayStart; i < clipper.DisplayEnd; i++) {
				...
				time =
				    max(ceil(round((old_value.timestamp + offset) * zoom + 0.5f) / zoom - offset),
				...
				while (true) {
					...
					if (new_screen_time > 0) {
						...
						if (lines_a.size() == 0) {
							if (var.is_vector()) {
								...
								lines_b.push_back(
								    base + ImVec2(
								               max(0, old_screen_time),
								               y_size * (1.0 - (int) old_value.type)));
							} else {
								lines_a.push_back(
								    base + ImVec2(
								               max(0, old_screen_time),
						...
						if (previous.timestamp > old_value.timestamp) {
							if (var.is_vector()) {
								lines_a.push_back(
								    base + ImVec2(max(0, previous_screen_time), y_size));
								lines_b.push_back(
								    base + ImVec2(
								               max(0, previous_screen_time),
								               y_size * (1.0 - (int) previous.type)));
							} else {
								lines_a.push_back(
								    base + ImVec2(
								               max(0, previous_screen_time),
						...
						if (var.is_vector()) {
							lines_a.push_back(
							    base + ImVec2(new_screen_time - FEATHER_SIZE / 2, y_size));
							...
							lines_b.push_back(
							    base + ImVec2(
							               new_screen_time - FEATHER_SIZE / 2,
							               y_size * (1.0 - (int) previous.type)));
							if (new_screen_time < (size_x - base.x)) {
								...
								lines_b.push_back(
								    base + ImVec2(
								               new_screen_time + FEATHER_SIZE / 2,
								               y_size * (1.0 - (int) value.type)));
							}
						} else {
							if (new_screen_time > 0.0f)
								lines_a.push_back(
								    base +
								    ImVec2(new_screen_time, y_size * (1.0 - (int) previous.type)));
							if (new_screen_time < (size_x - base.x)) {
								lines_a.push_back(
								    base +
								    ImVec2(new_screen_time, y_size * (1.0 - (int) value.type)));
							}
						}
					}
					...
					if (var.is_vector() and text_space > MIN_TEXT_SIZE and
						...
						auto end =
						    clip_text_to_width(text, text_space - 3 * PADDING - 2 * FEATHER_SIZE);
						draw->AddText(
						    base + ImVec2(
						               max(0, previous_screen_time) + 1 * PADDING + FEATHER_SIZE,
						               -PADDING),
					...
					if (value.timestamp >= last_time or new_screen_time >= (size_x - base.x)) {
						break;
					}
					...
					time =
					    max(ceil(next_pixel / zoom - offset),
